...
import { useListAssistants, useListConversations, getListAssistantsQueryKey, getListConversationsQueryKey } from "@workspace/api-client-react";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { Card, CardContent } from "@/components/ui/card";
import { MessageSquare, Calendar as CalendarIcon, User, Smartphone, Monitor } from "lucide-react";
...
export default function Conversations() {
  ...
  const { data: conversations, isLoading } = useListConversations(Number(selectedAssistant), {
    query: { 
      enabled: !!selectedAssistant,
      queryKey: getListConversationsQueryKey(Number(selectedAssistant))
    }
  });
          ...
          {conversations?.map((conv) => (
            <Card key={conv.id} className="bg-white/5 border-white/10 hover:border-primary/50 transition-colors cursor-pointer group">
              <CardContent className="p-6 flex items-center justify-between">
                ...
                <div className="hidden sm:block text-primary opacity-0 group-hover:opacity-100 transition-opacity font-medium text-sm">
                  View Chat &rarr;
                </div>
              </CardContent>
            </Card>
