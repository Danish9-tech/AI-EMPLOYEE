...
import { useListAssistants, useListAppointments, getListAssistantsQueryKey, getListAppointmentsQueryKey } from "@workspace/api-client-react";
...
export default function Appointments() {
  ...
  const { data: appointments, isLoading } = useListAppointments(Number(selectedAssistant), {
    query: { 
      enabled: !!selectedAssistant,
      queryKey: getListAppointmentsQueryKey(Number(selectedAssistant))
    }
  });
            ...
            <Card key={apt.id} className="bg-white/5 border-white/10 relative overflow-hidden">
              ...
              <CardContent className="p-6 pl-8">
                <div className="flex justify-between items-start mb-6">
                  ...
                  <Badge variant="outline" className={`${getStatusColor(apt.status)} uppercase tracking-wider text-[10px] font-bold`}>
                    {apt.status}
                  </Badge>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/10">
                  <div className="flex items-center text-sm text-white/80">
                    <User className="h-4 w-4 mr-2 text-white/40" />
                    {apt.name}
                  </div>
                  {apt.email && (
                    <div className="flex items-center text-sm text-white/80">
