...
import { Code, Copy, Key, MessageCircle, CheckCircle, XCircle, Loader2 } from "lucide-react";
...
export default function Settings() {
  ...
  const disconnectWhatsApp = async () => {
    ...
    try {
      const res = await fetch("/api/whatsapp/config", {
        method: "DELETE",
        headers: { "Clerk.User.User-Id": user?.id || "" },
      });
      ...
      <div className="grid gap-8">
        ...
        <Card className="bg-white/5 border-white/10">
          <CardHeader>
            ...
            <CardDescription>Managed securely via Clerk.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid gap-2">
              <Label className="text-white/80">Full Name</Label>
              <Input disabled value={user?.fullName || ""} className="bg-black/40 border-white/10 text-white/50" />
            </div>
            <div className="grid gap-2">
              <Label className="text-white/80">Email Address</Label>
              <Input disabled value={user?.primaryEmailAddress?.emailAddress || ""} className="bg-black/40 border-white/10 text-white/50" />
            </div>
            <Button variant="outline" className="border-white/10 text-white hover:bg-white/5" onClick={() => window.open(window.location.origin + '/user-portal', '_blank')}>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3 p-4 bg-green-500/10 border border-green-500/20 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Phone: {whatsappConfig.phoneNumber || "Not set"}
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Add this URL in your Meta Developer Portal → WhatsApp → Webhooks
                  </p>
                </div>
          ...
          <CardContent className="space-y-4">
            <div className="bg-black/60 p-4 rounded-lg border border-white/5 relative group">
              ...
              <Button
                size="sm"
                variant="ghost"
                className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity bg-black/50 text-white hover:text-primary hover:bg-black"
