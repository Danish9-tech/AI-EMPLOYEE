...
export default function AssistantNew() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: FormValues) => api.createAssistant(data),
    onSuccess: () => {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: { name: "", businessName: "", description: "", tone: "professional", widgetColor: "#00d4ff" },
  });
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <FormField control={form.control} name="description" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea placeholder="Describe what this assistant does and how it should behave..." className="min-h-[120px] bg-black/40 border-white/10 text-white" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              ...
              <div className="grid gap-6 md:grid-cols-2">
                ...
                <FormField control={form.control} name="widgetColor" render={({ field }) => (
                  <FormItem>
              ...
              <div className="flex justify-end pt-4 border-t border-white/10">
                <Button type="submit" className="bg-primary text-black hover:bg-primary/90" disabled={createMutation.isPending}>
                  {createMutation.isPending ? "Creating..." : (<><Save className="mr-2 h-4 w-4" /> Create Assistant</>)}
                </Button>
              </div>
            </form>
          </Form>
