  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-6xl mx-auto">
      ...
      <div className="grid md:grid-cols-3 gap-8">
        <PlanCard name="Free" price="$0" description="Perfect for testing and small personal projects."
          features={["100 messages/mo", "1 AI Assistant", "50 Leads/mo", "Web Widget only", "Community Support"]}
          current={sub?.plan === 'free'} />
        <PlanCard name="Pro" price="$49" period="/mo" description="For growing businesses needing serious automation."
          features={["Unlimited messages", "Up to 5 AI Assistants", "Unlimited Leads", "WhatsApp Integration", "Priority Support"]}
          highlight current={sub?.plan === 'pro'} />
        <PlanCard name="Enterprise" price="$199" period="/mo" description="Custom solutions for high-volume operations."
...
function PlanCard({ name, price, period = "", description, features, highlight = false, current = false }: any) {
  return (
    <Card className={`relative flex flex-col ${highlight ? 'bg-[#0a0a0f] border-primary/50 shadow-[0_0_30px_rgba(0,212,255,0.15)] transform md:-translate-y-4' : 'bg-white/5 border-white/10'}`}>
      {highlight && <div className="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-primary to-accent"></div>}
      <CardHeader>
        ...
        <div className="mt-4 flex items-baseline text-4xl font-extrabold text-white">
          {price}<span className="ml-1 text-xl font-medium text-muted-foreground">{period}</span>
        </div>
      </CardHeader>
      ...
      <CardFooter>
        <Button className={`w-full ${current ? 'bg-white/10 text-white hover:bg-white/20' : highlight ? 'bg-primary text-black hover:bg-primary/90' : 'bg-white text-black hover:bg-white/90'}`}
          disabled={current} variant={current ? "outline" : "default"}>
