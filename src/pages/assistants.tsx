...
export default function Assistants() {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: any }) => api.updateAssistant(id, data),
    onSuccess: () => {
  ...
  const toggleStatus = (id: number, currentStatus: boolean) => {
    updateMutation.mutate({ id, data: { isActive: !currentStatus } }, {
      onSuccess: () => {
        toast({ title: "Status updated", description: "Assistant status has been updated successfully." });
      },
      onError: () => {
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 bg-white/5 rounded-xl border border-white/10">
          ...
          <Link href="/assistants/new">
            <Button variant="outline" className="border-primary text-primary hover:bg-primary/10">Create Assistant</Button>
          </Link>
        </div>
      ) : (
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {assistants?.map((assistant: any) => (
            <Card key={assistant.id} className="bg-white/5 border-white/10 flex flex-col group hover:border-primary/50 transition-colors">
              ...
              <CardContent className="flex-1 flex flex-col">
                <p className="text-sm text-white/70 line-clamp-2 mb-6 flex-1">{assistant.description}</p>
                ...
                <div className="flex items-center gap-2 mt-auto pt-4 border-t border-white/10">
                  ...
                  <Button variant="ghost" size="sm" className="text-white hover:text-accent hover:bg-accent/10" onClick={() => showEmbed(assistant)}>
      ...
      <Dialog open={embedOpen} onOpenChange={setEmbedOpen}>
        <DialogContent className="bg-background border-white/10 text-white">
          <DialogHeader>
            ...
            <DialogDescription>Copy this code and paste it just before the closing &lt;/body&gt; tag of your website.</DialogDescription>
          </DialogHeader>
